...
#define FL_ERASE_ALL   (FL_ERASE_4K | FL_ERASE_32K | FL_ERASE_64K | FL_ERASE_CHIP)
...
struct sfc {
	...
	int (*chip_id)(struct sfc *ctrl, uint8_t *id_buf, uint32_t *id_size);
	int (*direct_read)(struct sfc *ctrl, uint32_t addr, void *buf,
			   uint32_t size);
	int (*write)(struct sfc *ctrl, uint32_t addr, const void *buf,
		     uint32_t size);
	int (*erase)(struct sfc *ctrl, uint32_t addr, uint32_t size);
	int (*cmd_rd)(struct sfc *ctrl, uint8_t cmd, bool has_addr,
		      uint32_t addr, void *buffer, uint32_t size);
	int (*cmd_wr)(struct sfc *ctrl, uint8_t cmd, bool has_addr,
