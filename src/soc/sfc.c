...
static inline int sfc_readl(struct sfc_data *ctx, uint32_t offset,
			    uint32_t *val)
{
	return soc_readl(ctx->soc, ctx->iomem.start + offset, val);
}

static inline int sfc_writel(struct sfc_data *ctx, uint32_t offset,
			     uint32_t val)
{
	return soc_writel(ctx->soc, ctx->iomem.start + offset, val);
}

static inline ssize_t flash_read(struct sfc_data *ctx, uint32_t offset,
				 void *buf, size_t len)
{
	return soc_read(ctx->soc, ctx->flash.start + offset, buf, len);
}

static inline ssize_t flash_write(struct sfc_data *ctx, uint32_t offset,
...
static int sfc_cmd_rd(struct sfc *ctrl, uint8_t cmd, bool has_addr,
		      uint32_t addr, void *buffer, uint32_t size)
{
	...
	if (buffer && size) {
		...
		/*
		 * Some bridges (P2A and debug UART, probably others too) have a
		 * quirk where they'll generate 4 byte reads even when a
		 * 1/2 byte read is requested. When the SFC is in user mode
		 * it'll clock out one byte for each byte of the MMIO read/write
		 * size as a result if we use anything smaller than a 4 byte
		 * read we'll lose data. The easiest solution is to just use 4
		 * byte reads for everything and extract the bytes manually
		 * when needed.
		 *
...
static int sfc_cmd_wr(struct sfc *ctrl, uint8_t cmd, bool has_addr,
		      uint32_t addr, const void *buffer, uint32_t size)
{
	struct sfc_data *ct = container_of(ctrl, struct sfc_data, ops);
	ssize_t rc;
...
static int sfc_direct_read(struct sfc *ctrl, uint32_t pos, void *buf,
			   uint32_t len)
{
	ssize_t rc;
...
static int sfc_check_reads(struct sfc_data *ct, const uint8_t *golden_buf,
			   uint8_t *test_buf)
{
	int i, rc;
...
#define FREAD_TPASS(i) (((i) / 2) | (((i) & 1) ? 0 : 8))
	...
	for (i = 0; i < 12; i++) {
		...
		SFC_DBG("  * [%08x] %d HCLK delay, %dns DI delay : %s\n",
			ct->fread_timing_val, i / 2, (i & 1) ? 0 : 4,
			pass ? "PASS" : "FAIL");
		if (pass) {
			pass_count++;
			if (pass_count == 3) {
...
static int sfc_setup_macronix(struct sfc_data *ct, struct flash_info *info)
{
	 ...
	 */
	ct->ctl_val = (ct->ctl_val & 0x2000) | (0x00 << 28) | /* Single bit */
...
static int sfc_driver_init(struct soc *soc, struct soc_device *dev)
{
	...
	if ((rc = soc_device_get_memory_index(soc, &dev->node, 0, &ct->iomem)) <
	    0)
		goto fail;

	if ((rc = soc_device_get_memory_index(soc, &dev->node, 1, &ct->flash)) <
	    0)
		goto fail;

	ct->type = (unsigned long)(soc_device_get_match_data(soc, sfc_match,
							     &dev->node));
	if (!ct->type) {
