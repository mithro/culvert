...
static int ast2400_ilpcctl_enforce(struct bridgectl *bridge,
				   enum bridge_mode mode)
{
	...
	if (mode == bm_disabled) {
		if ((rc = sioctl_decode_configure(ctx->sioctl,
						  sioctl_decode_disable)) < 0) {
	...
	if ((rc = soc_readl(ctx->soc, ctx->lpc.start + LPC_HICRB, &hicrb)) <
	    0) {
	...
	if ((rc = soc_writel(ctx->soc, ctx->lpc.start + LPC_HICRB, hicrb)) <
	    0) {
...
static int ast2400_ilpcctl_status(struct bridgectl *bridge,
				  enum bridge_mode *mode)
{
	struct ilpcctl *ctx = to_ilpcctl(bridge);
...
static int ilpcctl_report(struct bridgectl *bridge, int fd,
			  enum bridge_mode *mode)
{
	struct ilpcctl *ctx = to_ilpcctl(bridge);
	enum sioctl_decode decode;
...
static int ast2600_ilpcctl_enforce(struct bridgectl *bridge,
				   enum bridge_mode mode)
{
	struct ilpcctl *ctx = to_ilpcctl(bridge);
	uint32_t hicrb;
...
static int ast2600_ilpcctl_status(struct bridgectl *bridge,
				  enum bridge_mode *mode)
{
	struct ilpcctl *ctx = to_ilpcctl(bridge);
	enum sioctl_decode decode;
...
static const struct soc_device_id ilpcctl_matches[] = {
	...
	{ .compatible = "aspeed,ast2500-ilpc-ahb-bridge",
	  .data = &ast2400_ilpcctl_ops },
	{ .compatible = "aspeed,ast2600-ilpc-ahb-bridge",
	  .data = &ast2600_ilpcctl_ops },
	{},
};
...
static int ilpcctl_driver_init(struct soc *soc, struct soc_device *dev)
{
	...
	if (ops == &ast2600_ilpcctl_ops) {
		if ((rc = bridges_device_get_gate(soc, dev, &ctx->bridges,
						  &ctx->gate)) < 0) {
	...
	if ((rc = soc_bridge_controller_register(soc, ilpcctl_as_bridgectl(ctx),
						 ops)) < 0) {
