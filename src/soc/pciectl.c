...
enum device_function {
	...
	device_function_mmio,
	device_function_xdma
};
...
static uint32_t
pciectl_pdata_collect_region_mask(const struct pciectl_pdata *pdata,
...
static int pciectl_device_enforce(struct pciectl *ctx,
				  const struct pciectl_endpoint *ep,
				  enum bridge_mode mode)
{
	...
	if ((rc = soc_readl(ctx->soc, ctx->scu.start + ctx->pdata->config,
			    &pcie)) < 0) {
	...
	if (mode == bm_disabled) {
		...
		if ((rc = soc_writel(ctx->soc,
				     ctx->scu.start + ctx->pdata->config,
				     pcie)) < 0) {
	...
	if ((rc = soc_readl(ctx->soc, ctx->scu.start + ctx->pdata->misc,
			    &misc)) < 0) {
	...
	if ((rc = soc_writel(ctx->soc, ctx->scu.start + ctx->pdata->misc,
			     misc)) < 0) {
	...
	if ((rc = soc_writel(ctx->soc, ctx->scu.start + ctx->pdata->config,
			     pcie)) < 0) {
	...
	if (ctx->bridges) {
		...
		if ((rc = bridges_device_get_gate_by_name(
			     ctx->soc, ctx->dev, ep->gate, NULL, &id)) < 0) {
...
static int pciectl_device_status(struct pciectl *ctx,
				 const struct pciectl_endpoint *ep,
				 enum bridge_mode *mode)
{
	uint32_t pcie, misc;
...
static int pciectl_device_report(struct pciectl *ctx, int fd,
				 const struct pciectl_endpoint *ep)
{
	...
	dprintf(fd, "\t%s: %s\n", pcie_device_table[ep->device.type],
		description);
	if (!enabled) {
		return 0;
	}
...
static int pciectl_enforce(struct pciectl *ctx, enum bridge_mode mode,
			   enum device_function function)
{
	const struct pciectl_endpoint *curr;
	int rc;

	for (curr = ctx->pdata->endpoints;
	     curr->function.type != device_function_none; curr++) {
		...
		if ((rc = pciectl_device_enforce(ctx, curr, mode)) < 0) {
			loge("Failed to enforce bridge state for %s %s: %d\n",
			     pcie_device_table[curr->device.type],
			     device_function_table[curr->function.type], rc);
			return rc;
		}
	}
...
static int pciectl_status(struct pciectl *ctx, enum bridge_mode *mode,
			  enum device_function function)
{
...
static int pciectl_report(struct pciectl *ctx, int fd,
			  enum device_function function)
{
	const struct pciectl_endpoint *curr;
	int rc;
...
{
	return pciectl_enforce(p2actl_to_pciectl(bridge), mode,
			       device_function_mmio);
}

static int p2actl_status(struct bridgectl *bridge, enum bridge_mode *mode)
{
	return pciectl_status(p2actl_to_pciectl(bridge), mode,
			      device_function_mmio);
}

static int p2actl_report(struct bridgectl *bridge, int fd,
			 enum bridge_mode *mode)
{
...
static int xdmactl_enforce(struct bridgectl *bridge, enum bridge_mode mode)
{
	...
	if ((rc = sdmc_configure_xdma(ctx->sdmc, (mode == bm_permissive))) <
	    0) {
...
static int xdmactl_status(struct bridgectl *bridge, enum bridge_mode *mode)
{
	...
	if ((rc = pciectl_status(xdmactl_to_pciectl(bridge), mode,
				 device_function_xdma)) < 0) {
...
static int xdmactl_report(struct bridgectl *bridge, int fd,
			  enum bridge_mode *mode)
{
	struct pciectl *ctx = xdmactl_to_pciectl(bridge);
	int rc;
...
static const struct soc_device_id pciectl_matches[] = {
	...
	{ .compatible = "aspeed,ast2500-pcie-device-controller",
	  .data = &ast2500_pdata },
	{ .compatible = "aspeed,ast2600-pcie-device-controller",
	  .data = &ast2600_pdata },
	{},
};
...
static int pciectl_driver_init(struct soc *soc, struct soc_device *dev)
{
	...
	if ((rc = soc_bridge_controller_register(soc, p2actl_as_bridgectl(ctx),
						 &p2actl_ops)) < 0) {
		goto cleanup_drvdata;
	}

	if ((rc = soc_bridge_controller_register(soc, xdmactl_as_bridgectl(ctx),
						 &xdmactl_ops)) < 0) {
		goto cleanup_drvdata;
	}
